...
if st.session_state.monitoring_stocks:
    ...
    for symbol, info in st.session_state.monitoring_stocks.items():
        try:
            current = analyzer.get_current_price(symbol, info['type'])
            if current:
                change = ((current - info['stats']['last_close']) / info['stats']['last_close']) * 100
                
                # 시그마 레벨 체크
                alert = ""
                if change <= info['stats']['3sigma']:
                    alert = "🚨🚨🚨 3σ"
                elif change <= info['stats']['2sigma']:
                    alert = "🚨🚨 2σ"
                elif change <= info['stats']['1sigma']:
                    alert = "🚨 1σ"
                ...
                current_prices.append({
                    ...
                    '현재가': f"{current:,.4f}",
                    '변화율': f"{change:+.2f}%",
                    '1σ(5년)': f"{info['stats']['1sigma']:.2f}%",
                    '2σ(5년)': f"{info['stats']['2sigma']:.2f}%", 
                    '3σ(5년)': f"{info['stats']['3sigma']:.2f}%",
                    '1σ(1년)': f"{info['stats'].get('1sigma_1y', info['stats']['1sigma']):.2f}%",
                    '2σ(1년)': f"{info['stats'].get('2sigma_1y', info['stats']['2sigma']):.2f}%",
                    '3σ(1년)': f"{info['stats'].get('3sigma_1y', info['stats']['3sigma']):.2f}%",
                    '상태': alert or "정상"
                })
            else:
                current_prices.append({
