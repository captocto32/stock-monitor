...
SERVICE_ACCOUNT_FILE = 'gen-lang-client-0213805963-ce3cb3dbd649.json'
...
def load_stocks_from_sheets():
    """Google Sheets에서 저장된 종목 목록 불러오기"""
    try:
        client = get_google_sheets_client()
        if not client:
        ...
        try:
            ...
            st.warning("Google Sheets 문서를 찾을 수 없습니다. 새로 생성됩니다.")
            return {}
...
def load_saved_stocks():
    """저장된 종목 목록 불러오기 (Google Sheets만 사용)"""
    sheets_data = load_stocks_from_sheets()
    if not sheets_data:
        st.info("📝 저장된 종목이 없습니다. 새로운 종목을 추가해보세요!")
    return sheets_data
    ...
    if 'current_analysis' in st.session_state:
        ...
        with col_a:
            ...
            if current:
                ...
                if analysis['type'] == 'KR':
                    st.metric("현재가", f"₩{current:,.0f}", f"{change:+.2f}%")
                else:
